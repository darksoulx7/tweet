...
class CommentService {
  public async addCommentToDB(commentData: ICommentJob): Promise<void> {
    ...
    const post: Query<IPostDocument, IPostDocument> = PostModel.findOneAndUpdate({ _id: postId }, { $inc: { commentsCount: 1 } }, { new: true }) as Query<IPostDocument, IPostDocument>;
    ...
    if (response[2].notifications.comments && userFrom !== userTo) {
      ...
      commentsEmailQueue.addEmailJob('commentsEmail', { receiverEmail: response[2].email!, template, subject: 'Post notification' });
    }
  }
