...
export class UserCache extends BaseCache {
    ...
    public async saveUserToCache(
        ...
        try {
            ...
            log.error(error);
            throw new ServerError('saveUserToCache Server error. Try again.');
        }
    }
    ...
    public async getUserFromCache(
        ...
        try {
            ...
            response.followersCount = Helpers.parseJson(
                `${response.followersCount}`,
            );
            response.followingCount = Helpers.parseJson(
                `${response.followingCount}`,
            );
            response.bgImageId = Helpers.parseJson(`${response.bgImageId}`);
            response.bgImageVersion = Helpers.parseJson(
                `${response.bgImageVersion}`,
            );
            response.profilePicture = Helpers.parseJson(
