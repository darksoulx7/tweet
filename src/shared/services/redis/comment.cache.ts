...
export class CommentCache extends BaseCache {
  ...
  public async getCommentsNamesFromCache(postId: string): Promise<ICommentNameList[]> {
    try {
      ...
      let response: ICommentNameList | undefined;
      if (commentsCount > 0) {
        response = {
          count: commentsCount,
          names: list,
  ...
  public async getSingleCommentFromCache(postId: string, commentId: string): Promise<ICommentDocument[]> {
    try {
      ...
      const result: ICommentDocument | undefined = find(list, (listItem: ICommentDocument) => listItem._id === commentId ) as ICommentDocument;
