...
import { addUserProfileImageQueue, updateBGImageQueue } from '@service/queues/image.queue';
...
export class Add {
  ...
  public async profileImage(req: Request, res: Response): Promise<void> {
    ...
    const url = `https://res.cloudinary.com/dyjgikbog/image/upload/v${result.version}/${result.public_id}`;
    ...
    addUserProfileImageQueue.addImageJob('addUserProfileImageToDB', { key: `${req.currentUser!.userId}`, value: url, imgId: result.public_id, imgVersion: result.version.toString(), });
  ...
  public async backgroundImage(req: Request, res: Response): Promise<void> {
    ...
    const bgImageId: Promise<IUserDocument> = userCache.updateSingleUserItemInCache(`${req.currentUser!.userId}`, 'bgImageId', publicId) as Promise<IUserDocument>;
    const bgImageVersion: Promise<IUserDocument> = userCache.updateSingleUserItemInCache(`${req.currentUser!.userId}`, 'bgImageVersion', version) as Promise<IUserDocument>;
    ...
    updateBGImageQueue.addImageJob('updateBGImageInDB', { key: `${req.currentUser!.userId}`, imgId: publicId, imgVersion: version.toString(), });
    res.status(HTTP_STATUS.OK).json({ message: 'Image added successfully' });
  }
